...
function logOpenAIAnalyzeError(error: unknown) {
...
function hasExpectedShape(value: unknown) {
...
export async function POST(request: Request) {
  ...
  try {
        ...
        input: [
          {
            ...
            content: [
              fileContent,
              {
                type: "input_text",
                text:
                  "请分析这份保险资料，并按 JSON Schema 返回 3 个最值得用户立刻确认的风险点。所有字段内容必须是简体中文。要求：1. 只依据文件事实；2. 每条都尽量引用具体金额、年份、比例、页内条件或条款限制；3. 不要建议买或退；4. 如果资料不完整，missingInfo 必须列出缺失材料；5. 每条都包含事实、为什么要注意、进一步确认什么；6. whyItMatters 必须举具体数字例子，尽量算出用户在某一年可能少拿多少钱，能和文件中的定存/保底/演示收益等对照时要写出差额，不能编造文件没有的数据。",
              },
            ],
          },
        ],
    ...
    if (!outputText) {
      ...
      return NextResponse.json({ error: formatError }, { status: 502 });
    }
    ...
    if (isTimeoutError(error)) {
      return NextResponse.json({ error: timeoutAnalyzeError }, { status: 504 });
    }
